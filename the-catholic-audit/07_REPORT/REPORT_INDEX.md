...
| LLM 2차 심사 (YES만) | [llm_verified_conflicts.csv](./llm_verified_conflicts.csv) | `scripts/llm_judge.py` — claude CLI 헤드리스 심사 (API 키 불필요). **상위 500건 전수 심사 완료: 진짜 충돌 후보 8건** (방향 오류 가드 적용, 에러 0건) | ✅ 최신 |
| LLM 심사 전체 로그 | [llm_judge_full_log.csv](./llm_judge_full_log.csv) | 상위 500건의 YES/NO 전체 판정 + 근거 (감사 추적용). 501위 이하 재개: `python scripts/llm_judge.py N 501` | ✅ 최신 |
