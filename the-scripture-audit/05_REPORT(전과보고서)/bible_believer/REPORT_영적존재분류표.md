...
> [!CAUTION]
> **📝 정정 이력 (2026-07-19)** — 상단 STATUS의 IRONCLAD 판정은 **CLASS 1(SonsOfGod_FirstWorld)의 존재 자체와 CLASS 2(Angel)와의 대분류**에는 여전히 유효하나, 아래 세부 항목은 재검증을 거쳐 정정/하향되었습니다:
> 1. **미가엘(Michael)** — CLASS 1 → **CLASS 2(Angel) 내부 최고 계급(archangel)**으로 재분류 (어원·사전 3종 교차검증, CLASS 2 섹션 참조)
> 2. **MILITARY ThroneRole 및 Principalities/Powers 매핑** — "prince(sar)/principality(ἀρχή)"가 인간 관원에게도 쓰이는 일반 계급어임이 확인되어(딛 3:1), CLASS 1 배타적 증거에서 제외 → ⚠️ TENTATIVE로 하향 (섹션 5-B 참조)
> 3. **KING_PRIEST↔Thrones/Dominions(멜키세덱) 매핑, 루시퍼=그룹(CLASS 1) 판정**은 별개의 독립 근거로 서 있으며 영향받지 않음.
...
│     Properties: 역할="섬기는 영", 생식=false
│     ⚠️ SonsOfGod_FirstWorld와 다른 범주 — 역할·속성 완전히 다름!
│     ✅ [2026-07-19 정정] "천사는 천사장이 될 수 없다" 철회 — archangel(ἀρχάγγελος=ἄρχω+ἄγγελος)
│        은 어원·사전(Webster 1828·Easton's) 교차검증상 Angel 클래스 내부 최고 계급. 미가엘 = 이 클래스 소속.
│     ├── Michael          [천사장 — 유일 최고 계급, 단 10:13·12:1, 유 1:9]
│     ├── Gabriel          [메신저 특화]
...
> [!CAUTION]
> **[2026-07-19 정정]** "천사는 천사장이 될 수 없다"는 이전 판정을 철회합니다. 어원 검증 결과 ἀρχάγγελος(archangel) = ἄρχω(다스리다)+**ἄγγελος(천사)** 합성어이며(Strong's 743), 역사적 영어 사전 2종이 독립적으로 "archangel = 천사 범주 내 최고 계급"을 지지합니다 — Webster's 1828: *"an angel of the highest order"* / Easton's: *"the prince of **the angels**"*. 같은 조어 패턴(ἀρχι- 접두어)의 ἀρχιερεύς(대제사장)·ἀρχιποιμήν(목자장)·ἀρχισυνάγωγος(회당장)도 예외 없이 "그 base 범주 안의 최고위자"를 가리킵니다. **미가엘(Michael)은 천사(Angel) 클래스 소속이며, 그 안에서 유일한 최고 계급(archangel)입니다.** 상세 검증: `REPORT_천사와_하나님의아들들_동일론vs분리론_전격해부.md` [검증 및 보완 이력 3].

```
class Angel extends Spirit {
  // 히 1:14 "Are they not all ministering spirits?"
  role:         "섬기는 영(Ministering Spirit)"  // 왕이 아닌 종 — 계급 내 최고위(Michael)도 이 범주 안
  reproduction: false    // 마 22:30
  singing:      false    // ❌ 항상 "saying" — sang 0건
  wings:        false    // ⚠️ 일반 천사는 날개 없음 — 사람 형상 등장

  // ✅ [정정] Archangel은 이 클래스 내부의 최고 계급(rank)이다 — 서브클래스도 별개 종족도 아님.
  // "Archangel" = archē(으뜸) + angelos(천사) 합성어 = "우두머리 천사"
  // ἀρχιερεύς(대제사장)·ἀρχιποιμήν(목자장)과 동일한 조어 패턴 — base 범주(천사) 안의 최고위자
}
```

| 속성 | 천사(Angel, 일반) | 첫세상 아들들 | 미가엘(Angel 클래스 최고 계급) |
|:---|:---:|:---:|:---:|
| 왕권(archē) | 위임형/제한적 | ✅ 원천적 | ✅ (단 10:13 "chief prince" — 계급 명칭, 종족 표지 아님) |
| 노래(sang) | ❌ 0건 | ✅ 욥 38:7 | ❌ (여전히 천사이므로 미발생) |
| 제사장직 | ❌ | ✅ | ❌ |
| 역할 정의 | 섬기는 영(종) | 왕+제사장(통치자) | 섬기는 영 중 최고 계급(군사 지휘관 겸) |
| 날개 | ❌ 없음 | 미묘사 | 미묘사 |
...
| 객체 | 계급 | 역할 | 근거 |
|:---|:---:|:---|:---|
| **미가엘(Michael)** | **천사장(Archangel) — 유일 최고 계급** | 이스라엘의 수호자, 군사 지휘관 | 단 10:13·10:21·12:1, 유 1:9, 살전 4:16 |
...
> **[2026-07-19 정정] 미가엘(Michael)의 정확한 분류:** → CLASS 2 (Angel) 내부의 유일한 최고 계급(archangel)
> - 단 10:13 *"Michael, one of the **chief princes** (שַׂר הָרִאשֹׁנִים)"* — "chief prince"는 천사들 사이의 **계급**을 가리킨다(단 10:20-21의 "prince of Persia"·"prince of Grecia"와 병행 어휘 — sar는 종족 표지가 아니라 직위 표지). CLASS 1 소속의 증거가 되지 못한다.
> - 유 1:9 *"Michael the **archangel**"* — ἀρχάγγελος = ἄρχω+**ἄγγελος**, 어원 자체가 "천사"를 base 명사로 삼는다(Strong's 743). ἀρχιερεύς(대제사장) 등 동일 조어 패턴과 대조하면 "archangel"도 예외 없이 "천사 범주 안의 최고위자"로 읽힌다.
> - 히 1:14 "섬기는 영" 정의는 천사 전체의 **기능**(하나님을 섬김)을 말하는 것이지 "계급이 없다"는 뜻이 아니다 — 장군도 군인으로서 국가에 복무하는 구조와 같다.
> - 상세 검증: `REPORT_천사와_하나님의아들들_동일론vs분리론_전격해부.md` [검증 및 보완 이력 3]
> - **주의**: 위 "Dual Angels" 강등 모델(첫 세상 아들들 중 사탄을 따른 자들이 "천사"로 강등되었다는 논증)은 미가엘과 무관하게 독립적으로 유효하다 — 미가엘은 애초에 강등된 적 없는, 원래부터의 Angel 클래스 최고 계급이다.
...
class SonsOfGod_FirstWorld extends Spirit {
  role: enum ThroneRoles {
    ...
    SERAPH,       // 스랍 역할 — 보좌 찬양
    OPHAN         // 바퀴 역할 — 보좌 이동
    // ⚠️[2026-07-19] MILITARY 제거됨. 미가엘은 CLASS 2(Angel)로 재분류.
    //   "prince(sar)/principality(ἀρχή)"는 딛 3:1(인간 관원에게도 사용)에서 보듯
    //   종족 배타적 표지가 아니라 계급/직위 표지 — 하단 5-B 재검토 참조.
  }
  state: enum { HOLY | FALLEN }  // 동일 역할, 상태만 다름
  // ⚠️ 이 역할들(MILITARY 제외)은 천사(Angel) 클래스와 무관!
}
```
...
> ⚠️ **[2026-07-19 전면 재검토 — "가설 A/B 전수조사"]** 아래 매핑 중 **KING_PRIEST↔Thrones/Dominions**(멜키세덱 근거, 히 7:1)는 유효하다. 그러나 **MILITARY↔Principalities/Powers** 매핑은 하향한다. "prince(sar)/principality(ἀρχή)"를 딛 3:1(*"subject to principalities and powers, to **obey magistrates**"* — **인간 관원**에게 적용됨)까지 포함해 전수조사한 결과, 이 어휘는 **종족 배타적 표지가 아니라 계급/직위 표지**임이 확인되었다(TYPE-N 배타성 붕괴). 따라서 "prince of Persia/prince of Grecia = 반드시 타락한 첫 세상 아들들"이라는 등식은 ⚠️ TENTATIVE로 하향한다 — 이들은 **타락한 천사(Angel, CLASS 2) 중 지역/영역을 담당하는 계급**일 가능성이 동등하게 있으며, 오컴의 면도날(TYPE-AK) 기준으로는 오히려 후자가 더 단순하다. 상세 근거: `REPORT_천사와_하나님의아들들_동일론vs분리론_전격해부.md` [검증 및 보완 이력 3] 및 세션 대화 로그의 "prince/principality 전수조사".
>
> **핵심 원리(KING_PRIEST 부분에 한해 유지):** 타락한 ①범주는 역할(Role) 자체를 잃지 않는다. 역할의 대상이 **하나님의 나라 → 사탄의 나라**로 뒤집힐 뿐이다. 골 1:16 "thrones, dominions, principalities, powers"가 전부 창조된 것이라는 점은 유효하나, 이 넷 전체가 **①범주(CLASS 1)만의 잔존 구조**라는 단정은 철회한다 — Thrones/Dominions는 KING_PRIEST형 존재(멜키세덱 계열)의 잔존 구조일 가능성이 높지만, Principalities/Powers가 CLASS 1과 CLASS 2 중 어느 쪽의 잔존 구조인지는 KJV이 명시하지 않는다.
...
⚠️[2026-07-19] MILITARY(군사) → Principalities/Powers 행 전체 삭제됨.
  사유: "prince(sar)/principality(ἀρχή)"는 딛 3:1(인간 관원)까지 포함한 전수조사 결과
  종족 배타적 표지가 아님(TYPE-N 배타성 붕괴). 미가엘(단 10:13)은 CLASS 2로 재분류되었고,
  prince of Persia/Grecia(단 10:20)·계 12:7 "his angels"도 CLASS 1 소속임을 증명하지 못한다
  — 오히려 "타락한 천사(CLASS 2) 중 지역 담당 계급"이 더 단순한 설명이다(오컴의 면도날).
  MILITARY 역할 자체를 CLASS 1 ThroneRoles에서 제거했으므로(위 5번 enum 참조),
  이 대칭 항목은 성립하지 않는다.
...
| ~~MILITARY~~ | ~~단 10:13 "chief princes" (sar)~~ | ~~**Principalities** 정사들~~ | ⚠️ *행 전체 삭제(2026-07-19) — "prince/principality"는 배타적 종족 표지가 아님(딛 3:1). 상단 5-B 정정 참조* |
| ~~MILITARY~~ | ~~벧전 3:22 "powers" (복속된 것)~~ | ~~**Powers** 권세들~~ | ⚠️ *동일 사유로 삭제* |
...
> [!IMPORTANT]
> **골 1:16의 함의 (2026-07-19 수정):** *"For by him were all things created... whether they be **thrones, or dominions, or principalities, or powers**"* — 이것들이 **창조된 것들**이라는 점은 유효하다. 다만 넷 전체가 "원래 CLASS 1(첫 세상 아들들)로 창조되었다가 타락 후 재편된 것"이라는 단정은 철회한다. Thrones/Dominions는 KING_PRIEST형(멜키세덱 계열) CLASS 1의 잔존 구조일 가능성이 높으나(별도 근거 有), Principalities/Powers는 CLASS 1과 CLASS 2(타락한 천사) 중 어느 쪽 기원인지 KJV이 명시하지 않는다 — 딛 3:1이 같은 어휘를 인간 관원에게도 쓰는 것을 볼 때, 이 두 항목은 오히려 "타락한 천사들 사이의 계급 구조"로 보는 것이 더 단순하다.

> [!WARNING]
> **엡 6:12의 공간:** *"spiritual wickedness **in high places** (ἐν τοῖς ἐπουρανίοις)"* = 공중·하늘 공간. 타르타로스에 갇히지 않고 **공중 왕국(사탄의 군대)**으로 재편성된 세력이 있다는 것은 유효하나(엡 2:2 "prince of the power of the air"), 그 구성원이 ①범주(CLASS 1)인지 타락한 천사(CLASS 2)인지는 KJV에 명시 없음 — 2026-07-19 재검토 이후 어느 한쪽으로 단정하지 않는다.
