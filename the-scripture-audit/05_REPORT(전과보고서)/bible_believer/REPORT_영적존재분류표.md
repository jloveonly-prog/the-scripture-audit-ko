...
> **발견적 통찰:** 성경에서 '천사(Angelos/Malakh)'는 종족명(Species)이 아니라 기능과 직분(Functional Office)을 뜻하는 **인터페이스(Interface)**입니다. 이로 인해 성경에는 기원이 전혀 다른 두 존재가 '천사'라는 동일한 호칭으로 불립니다.
...
class DemotedOrHumanAngel implements Angelos {
  ...
  //    '육체(Flesh)'가 되고 수명이 120년으로 단축된 타락한 '인간 왕'들임.
}
```
