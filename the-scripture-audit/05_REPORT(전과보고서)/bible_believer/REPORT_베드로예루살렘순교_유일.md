...
> **STATUS (2026-07-24 명제 분리)**: 이 문서는 두 개의 다른 정밀도를 가진 명제를 함께 다룬다. 아래 두 줄을 혼동하지 말 것.
> - **명제 1 (도시 — 예루살렘)**: 🟢 **IRONCLAD (강함)** / 목회 적용 = 🟡 개연성 [Tier 1(1·8·16번, 4개 독립 저자) 단독으로 성립 — Tier 2 불필요]
> - **명제 2 (특정 처형지 — 갈보리/골고다)**: 🟢 **IRONCLAD** / 목회 적용 = 🟡 **개연성(Probability)** [교차증인 미확인 — Tier 1 + Tier 2(요한복음 내부 ὅπου→τόπος 락인 등) 전체 필요, 2026-07-19 조정, 하단 CAUTION 박스 참조]
> **등급 정의**: ✅ **EXPLICIT**(직접 명시) = 성경이 직접 기록한 사실 (예: 베드로의 십자가형, 요 21:18). 🟢 **IRONCLAD**(추론 철벽) = 성경이 직접 명시하지는 않았으나, 모든 대안 해석이 성경 내부 모순을 발생시키므로 **유일하게 모순 없이 성립하는 해석** (예: 순교 도시 = 예루살렘, 순교 지점 = 갈보리). ⚠️ 두 명제 모두 EXPLICIT이 아니다 — 신약 어디에도 "베드로가 예루살렘/갈보리에서 죽었다"는 문장 자체는 없다. 자세한 등급 근거는 위 STATUS 두 줄과 하단 "🏗️ 논증 계층 구조" 참조.
...
## 🏆 총괄 요약: 모든 경우의 수 및 신학 점수표

| 항목 (증거명) | 핵심 성경 구절 | 교차 검증 구절 | 관련성 설명 (왜 갈보리와 락인되는가?) | 신학 점수 |
|---|---|---|---|---|
| **1. "별세(Exodos)" 앵커** | 벧후 1:14-15 | 눅 9:31 | 신약에서 'Exodus(decease)'가 죽음으로 쓰인 곳은 단 두 곳(예수님의 예루살렘 죽음, 베드로의 죽음)뿐임. 성령이 눅 9:31에서 예루살렘에 결박해 둔 독점 어휘를 베드로에게 그대로 적용한 완벽한 교차 확정. | **🟢 확정 (독점어휘/교차)** |
| **2. "발자취(ἴχνος)" 추적** | 벧전 2:21 | 요 21:19, 벧전 2:24 | "따르라"는 명령을 주님이 십자가 나무를 향해 걸어가신 물리적 발자국으로 해석하는 신학적 은유. | **🟡 개연성 (은유/추론)** |
| **3. 좌표 지시 패턴 배타성** | 요 21:19 | 마 14:29, 마 16:23 | 주님의 이동 지시가 항상 좌표를 동반했다는 귀납적 패턴에 근거하여 "따르라"를 장소로 해석. | **🟡 개연성 (패턴/추론)** |
| **4. 장막(σκηνή)의 역전** | 벧후 1:13-14 | 마 17:4 | 변화산의 자발적 장막 치기가 갈보리의 수동적 장막 벗기로 역전된다는 완벽한 문학적 대칭. | **🟡 개연성 (예표/대칭)** |
| **5. 두 시몬의 대리 예표** | 요 13:36 | 눅 23:26, 마 27:32 | 베드로가 당장 질 수 없던 십자가를 동일한 이름의 '시몬'이 갈보리로 대리해서 짐. | **🟡 개연성 (예표/추론)** |
| **6. 구속사적 지리 대칭** | 행 12장 (베드로) | 행 23:11 (바울) | 바울의 목적지가 이방 로마로 강제된 것처럼, 할례자의 사도 베드로는 예루살렘에 속한다는 지리적 대칭. | **🟡 개연성 (대칭/추론)** |
| **7. 유대주의 인력(Gravity)** | 갈 2:11-14 | 갈 2:8 | 베드로의 영적/심리적 두려움의 대상은 철저히 예루살렘 유대주의에 묶여 있다는 사역 궤적 추론. | **🟡 개연성 (심리/추론)** |
| **8. 선지자 예루살렘 3중 콤보** | 눅 13:33-34 | 요 21:19, 요 1:42, 벧전 2:5, 마 26:34 | 눅 13장 33절의 절대 명제("선지자는 예루살렘에서 죽는다")와 요 21:19(베드로의 순교 예언)이 교차하며, 직후인 34절에 베드로를 상징하는 **살인 무기(Stone)**와 **가장 뼈아픈 3번 부인 사건의 예표(수탉-암탉의 울음)**가 동시에 3중 콤보로 터져 장소를 예루살렘으로 완벽히 락인시킴. | **🟢 확정 (3중 콤보)** |
| **9. 역논법 (지금은/후에는)** | 요 13:33 | 요 13:36 | "내가 가는 곳"에 유대인은 못 가고 베드로는 후에 간다는 구조적 락인. (단, 일반 신학계에서는 이를 지리적 장소가 아닌 '영적 순교'라는 은유로 취급하므로 엄격한 기준에서 추론으로 분류함). | **🟡 개연성 (역논법/추론)** |
| **10. ὅπου→τόπος 골고다 락인** | 요 13:36 | 요 19:17-20 | 주님이 가시는 '그곳(ὅπου)'이 요 19:20에서 십자가에 달리신 '장소(τόπος/골고다)'와 원어상 완벽히 결합하며 갈보리를 특정함. | **🟡 개연성 (원어/추론)** |
| **11. '영광(δόξα)' 면류관 배타성** | 요 21:19 | 벧전 5:4 | '영화롭게 할(δοξάσει) 죽음'을 예언받은 베드로만이 신약 사도 중 유일하게 '영광(δόξης)의 면류관'을 록인(Lock-in)하여 어휘를 일치시킴. | **🟡 개연성 (어휘/배타성)** |
| **12. '띠(ζώννυμι)+따름' 완벽 대칭** | 요 13:4, 행 12:8 | 요 21:18-19 | 주님이 '띠 띠신' 것처럼 베드로도 순교를 위해 '띠 띠워'짐. 특히 행 12장 구출("스스로 띠 띠고 따르라")이 요 21장 순교("남이 띠 띠우고 따르라")와 원어상 완벽한 대칭(ζώννυμι+ἀκολούθει)을 이룸. | **🟡 개연성 (어휘/대칭)** |
| **13. 예루살렘 경로 재가동** | 마 16:21-24 | 요 21:19 | 예루살렘행 십자가 경로를 막아섰다가 책망받은 베드로에게 "나를 따르라" 명하심으로써 중단되었던 물리적 예루살렘행을 재가동하심. | **🟡 개연성 (구조/추론)** |
| **14. 요나의 표적 (지리적 궤적)** | 마 16:17 | 벧전 5:13 | 서방에서 동방 니느웨로 파송된 요나처럼, 서방(갈릴리)에서 동방(바빌론)으로 파송되었다가 십자가를 지러 서방(갈보리)으로 귀환하는 지리 예표. | **🟡 개연성 (예표/추론)** |
| **15. 발 씻기심 신체 부위 예표** | 요 13:7-9 | 요 21:18 | 요 13장에서 베드로가 씻겨달라 한 3부위(손/발/머리)는 정확히 십자가의 상처 부위(손발의 못, 머리의 가시면류관)를 예표하며, "지금은 모르나 이후에 알리라"는 말씀이 십자가 순교로 완벽히 성취됨. | **🟡 개연성 (예표/대칭)** |
| **16. 공관복음 3중 서원 (동반 죽음)** | 마 26:35, 막 14:31, 눅 22:33 | 요 13:37 | 세 제자가 공통으로 증언하듯, 베드로가 이해한 "따라옴"은 은유가 아니라 '물리적으로 같은 죽음에의 동행'이었고, 주님은 이 범주를 승인하셨음. | **🟡 개연성 (원청중/추론)** |
| **17. 증인(μάρτυς) 자격의 전환** | 요 19:25 | 벧전 5:1 | 십자가 현장에 없어 '시각 증인(목격)'이 되지 못한 베드로가 '고난의 증인'이 되는 유일한 길은, 갈보리 십자가 고난에 물리적으로 '참여'하는 것뿐임. | **🟡 개연성 (어휘/구조)** |
| **18. '어떠한 죽음(what death)' 독점** | 요 12:33, 18:32 | 요 21:19 | 신약 전체에서 "어떠한 죽음으로(σημαίνων ποίῳ θανάτῳ/what death)" 죽을지 예언하는 공식은 오직 예수님의 십자가 죽음(2회)과 베드로의 죽음(1회)에만 배타적으로 독점 사용됨. | **🟡 개연성 (어휘/배타성)** |

### 📊 최종 신학 점수 집계 (2026-07-24 개정 — Tier 구조·STATUS 이중등급과 정합화)
*   **복수 저자 교차증인 확보 (Tier 1):** 3개 — 1번(Exodos: 누가+베드로), 8번(눅13장 콤보: 누가+요한+마태), 16번(공관3중서원: 마태+마가+누가+요한, 4개 독립 저자)
*   **요한복음 내부 어휘·문법 콤보 수렴 (Tier 2):** 7개 — 저작 의도의 내부 일관성 근거이나 복수 저자 교차증인은 아님
*   **예표·정황 (Tier 3):** 8개 — Tier 1·2를 전제로 해서만 의미를 갖는 배경 증거
*   **명제 1 (도시 = 예루살렘) 결론:** 🟢 IRONCLAD — **Tier 1(복수 저자 3개)만으로 성립**, Tier 2·3 불필요. 목회 적용: 🟡 개연성이나 세 명제 중 가장 강함.
*   **명제 2 (특정 처형지 = 갈보리/골고다) 결론:** 🟢 IRONCLAD — Tier 1 + Tier 2 전체(요한복음 ὅπου→τόπος 락인 등) 필요. 목회 적용: 🟡 개연성(교차증인 미확인).
*   **판정 사유:** IRONCLAD 판정의 근거는 "증거 항목이 18개나 된다"는 숫자가 아니다. 항목 수는 목회 적용 등급을 자동으로 올려주지 않는다(위 CAUTION 박스 참조). 실제 근거는 "🔬 4. 역가설 검증" 섹션이 보여주듯, '따라옴'을 지리적 장소가 아닌 은유로 읽을 경우 **νῦν(지금은) 시간부사가 무의미해지고, σύ(오직 너는) 배타적 강조 대명사가 설명 불가능해지며, 요 13:33과 13:36이 동시에 참일 수 없게 되는** 등 대안 해석마다 성경 내부에 새로운 모순이 발생한다는 사실이다. 즉 "증거가 많다"가 아니라 "대안은 전부 본문과 충돌한다"는 소거법이 IRONCLAD의 근거다. 이 결론은 성경 텍스트가 직접 쓴 문장(EXPLICIT)이 아니라 추론(IRONCLAD)이므로, 목회 적용 등급은 확정(Doctrine)이 아닌 개연성(Probability)으로 표기한다.

> [!CAUTION]
> **📝 요약·인용 시 필수 고지 (2026-07-24 신설) — "18개 항목" ≠ "18명의 독립 증인"**
> 이 문서를 목회자·학자·외부 AI 등 제3자에게 요약해 전달하거나 평가를 의뢰할 때, **"18개의 증거가 수렴한다"는 표현만 단독으로 전달하지 말 것.** 실제 저자 구성을 확인하면:
> - **복수 저자 교차증인 확보 항목**: 1번(눅 9:31=누가 + 벧후 1:14-15=베드로 자신), 8번(눅 13장=누가 + 요 21장/요 1:42=요한 + 마 26:34=마태), 16번(마·막·눅=마태·마가·누가 + 요 13:37=요한, 단 "물리적 동행" **범주**의 증인이지 "갈보리" **지명**의 증인은 아님) — 이 3개만 진짜 복수 저자.
> - **나머지 15개 항목**은 압도적으로 **요한복음(+베드로 자기 서신) 내부**의 반복 신호다. 베드로 자신의 서신(벧전·벧후)은 사건 당사자의 자기 증언이므로 제3의 독립 저자로 카운트하지 않는다(11번 섹션 참조).
> - 따라서 "18개 항목이 동시에 수렴한다"는 서술은 **저작 의도의 내부 일관성(IRONCLAD 내부 등급의 근거)**으로는 정당하나, **"18개의 독립 반증이 있어야 논증이 무너진다"는 확률적 불가침성 주장으로 재구술되면 과장이다** — 실제로는 "요한의 저작 의도"라는 단일 전제 하나가 흔들리면 15개 항목이 함께 흔들릴 수 있다.
> - **요약 전달 권장 문구 예시**: *"18개 항목 중 2개는 🟢 확정(복수 저자 교차), 16개는 🟡 개연성(대부분 요한복음 단일 저자 내부 설계). 내부 논증 일관성은 IRONCLAD이나, 목회 적용 등급은 개연성(교차증인 미확인)."*

---

## 🏗️ 논증 계층 구조 (Tiered Argument Structure) — 2026-07-24 신설

> 위 18개 항목을 평면 나열이 아니라, **등급(🟢/🟡)과 저자 독립성**(위 CAUTION 박스 기준)을 그대로 반영해 3단으로 계층화한다. 새 증거를 추가하는 것이 아니라 기존 18개를 재배열한 것이며, 각 항목의 근거·구절·점수는 위 총괄 요약표를 그대로 따른다.

### Tier 1 — 핵심 기둥 (🟢 확정 + 복수 저자 교차증인)

| # | 항목 | 저자 구성 | 증명하는 것 |
|:---:|:---|:---|:---|
| 1 | Exodos 앵커 | 누가(눅 9:31) + 베드로(벧후 1:14-15) | 베드로의 죽음이 예수님의 예루살렘 죽음과 동일한 신약 유일 독점 어휘로 성령에 의해 결박됨 |
| 8 | 선지자 예루살렘 3중 콤보 | 누가(눅 13장) + 요한(요 21장·요 1:42) + 마태(마 26:34) | "선지자는 예루살렘에서 죽는다"는 절대 명제 + 베드로 정체성(돌/수탉) 락인 |
| 16 | 공관복음 3중 서원 | 마태(마 26:35) + 마가(막 14:31) + 누가(눅 22:33) + 요한(요 13:37) — 4개 독립 저자 | "따라옴" = 은유가 아닌 **물리적 죽음 동행**이라는 범주 확정 — ⚠️ 단, "갈보리"라는 특정 지명 자체는 증명하지 않는다(Tier 2의 역할) |

> Tier 1 세 항목만으로: ① 베드로의 순교 예언은 다른 제자와 무관한 배타적 예언이었고(1·8), ② 그 성취는 은유가 아니라 주님과 동일한 물리적 죽음 경로였다(16)는 것이 **4개 독립 저자**로 뒷받침된다. 다만 그 물리적 경로의 **종착지가 정확히 갈보리**라는 좌표 확정은 Tier 1 단독으로는 성립하지 않는다 — 이는 Tier 2(요한복음 내부 어휘·문법 콤보)가 담당한다.

### Tier 2 — 보강 기둥 (요한복음 내부 어휘·문법 콤보, 다중 수렴)

| # | 항목 |
|:---:|:---|
| 9 | 역논법 (지금은/후에는, νῦν/ὕστερον) |
| 10 | ὅπου→τόπος 골고다 락인 (COMBO-GN14) |
| 11 | 영광(δόξα) 면류관 배타성 (COMBO-SN12) |
| 12 | 띠(ζώννυμι)+따름 완벽 대칭 |
| 13 | 예루살렘 경로 재가동 |
| 17 | 증인(μάρτυς) 자격의 전환 |
| 18 | "어떠한 죽음"(σημαίνων ποίῳ θανάτῳ) 독점 |

> 요한 한 저자 내부에서 어휘·문법·구조가 반복적으로 같은 결론(갈보리라는 특정 좌표)을 가리키는 항목군이다. 저작 의도의 내부 일관성(IRONCLAD 내부 등급의 근거)은 강하나, 복수 저자 확률 배제 논증(위 TYPE-AJ STEP 3)에는 사용하지 않는다.

### Tier 3 — 예표·정황 (귀납·유비, 목회적 색채)
...
| 6 | 구속사적 지리 대칭 |
| 7 | 유대주의 인력(Gravity) |
| 14 | 요나의 표적 (지리적 궤적) — ⚠️ 요나서 본문 자체는 귀환 방향을 서술하지 않음(1단계 방어 정직 고지 참조) |
| 15 | 발 씻기심 신체 부위 예표 |

> 개별 강도는 가장 약하나, Tier 1·2가 이미 확립한 결론과 정합적인 배경 색채를 더한다. 이 항목들만으로는 독자적 결론을 도출할 수 없으며, Tier 1·2를 전제로 해서만 의미를 갖는다.

---

### 🛡️ 일반 신학계의 예상 반론과 팩트 폭격 (방어선 구축)

로마 가톨릭 전승에 영향을 받은 일반 신학자들이 이 문서의 핵심 확정 증거(1번, 8번)를 어떻게 공격할 것인지, 그리고 어떻게 팩트로 분쇄할 것인지 대비한다.

#### [방어선 1] 1번 증거 (Exodus/decease) 반론
*   **신학자들의 변명:** "베드로가 '별세(Exodus)'라는 단어를 예수님과 똑같이 쓴 것은 맞다. 하지만 그것은 지리적 장소(예루살렘)를 맞춘 것이 아니라, 자신의 죽음도 예수님처럼 '영광스러운 천국으로의 떠남(영적 엑소더스)'이라는 '신학적 의미'를 모방한 은유일 뿐이다."
*   **우리의 팩트 폭격:** 성경 어디에도 헬라어 원어 **'Exodus(ἔξοδος)'**와 영어 KJV의 엄밀한 번역어 **'decease'**를 물리적 장소와 분리해서 영적으로만 해석하라는 지시가 없다. 신약 성경 전체에서 이 원어와 영단어가 '죽음'을 뜻하는 단어로 짝을 이루어 사용된 곳은 오직 눅 9:31(예수님)과 벧후 1:15(베드로) 단 두 곳뿐이다. 눅 9:31은 명확히 "예루살렘에서 이루실 별세(decease)"라며 장소를 앵커(Anchor)로 굳게 박아두었다. 성령께서 원어(헬라어)와 보존된 영어(KJV) 양쪽 모두에서 2중으로 락인(Lock-in)해 둔 이 완벽한 단어 교차를 장소와 억지로 분리하는 것은, 신학자들이 로마 전승을 지키기 위해 성경 텍스트를 자의적으로 단절시키는 행위일 뿐이다.

#### [방어선 2] 8번 증거 (선지자 예루살렘 법안 3중 콤보) 반론
*   **신학자들의 변명:** "예수님이 눅 13:33에서 '선지자는 예루살렘 밖에서 죽지 못한다'고 하신 것은, 그 당시 유대인들의 악함을 고발하기 위한 '수사학적 과장(Proverb/속담)'일 뿐이다. 구약 선지자들을 예루살렘이 많이 죽였으니, 예수님 자신도 거기서 죽을 것이라는 뜻이지, 베드로 같은 신약의 사도들에게까지 영원히 적용되는 '절대 법안'이 아니다."
*   **우리의 팩트 폭격 (3중 콤보의 완벽한 락인):** 눅 13장 33-34절은 결코 단순한 속담으로 치부할 수 없는 정교한 **3중 콤보(절대 명제 + 돌 + 암수 닭)**로 엮여 있다.
    1. **절대 명제 (순교 장소):** 예수님은 "거의 없다"가 아니라 **"불가능하다(it cannot be)"**라며 선지자의 죽음을 예루살렘으로 법적으로 결박하셨다. 요 21장에서 순교 예언을 받은 베드로는 명백한 신약의 '선지자'다.
    2. **살인 무기와 정체성 (돌/Stone):** 33절 법안 직후 34절에서 예루살렘은 선지자를 **'돌로 치는(stonest/λιθοβολέω)'** 자로 묘사된다. 예수님이 베드로에게 지어주신 이름 **'돌(A stone/Πέτρος)'**은 KJV 영어 "stone" 번역에서만 겹칠 뿐, 헬라어 원어로는 πετρ- 어근으로 λιθοβολέω의 λιθ- 어근과 별개다(2026-07-24 정정 — 어근 대조 없이 "헬라어로도 일치"라 표기했던 구판 오류). 반면 베드로가 스스로 칭한 정체성 **'산 돌(lively stones/λίθοι, 벧전 2:5)'**은 λιθοβολέω와 헬라어 어근(λιθ-)까지 실제로 동일하여, 이 한 단어만이 예루살렘에서 피 흘릴 순교자를 어근 수준에서 락인(Lock-in)한다.
    3. **수탉과 암탉의 대칭 (Fowl):** 34절에서 예수님은 돌에 맞아 죽을 선지자(자신)를 새끼를 품으려는 **암탉(Hen, ὄρνις)**에 비유하셨다. 예루살렘은 이 암탉(예수님)을 거부하고 죽이는 도시다. 그런데 베드로가 예루살렘에서 예수님을 부인할 때 울었던 새는 정확히 대칭되는 **수탉(Cock, ἀλέκτωρ)**이었다.
*   **결론 (단어 대 단어의 4중 락인):** 구속사적인 의미 부여를 다 걷어내고 냉정하게 '단어 대 단어(Word for Word)' 텍스트로만 보더라도, 단 두 구절(눅 13:33-34) 안에 **① 순교지(예루살렘) + ② 선지자의 죽음 + ③ 베드로의 이름(돌/Stone) + ④ 베드로의 상징(닭/Fowl)**이라는 4개의 거대한 키워드가 하나의 문단 안에 완벽한 묶음으로 떨어진다. 성경 전체를 통틀어 이 4개의 단어가 이토록 정교하게 교차하는 곳은 오직 이곳뿐이다. 과연 이것을 '우연히 쓰인 속담'이라고 볼 확률이 수학적으로 존재하는가? 신학자들이 이 명백한 텍스트 묶음을 우연으로 치부하는 것은 성경 원어의 치밀한 영감을 정면으로 부정하는 것이다.
...
## 🔬 12-A. BVCAP 심층 전수조사 (2026-07-23 신규 증거)

> **STATUS**: 🟢 IRONCLAD 내부 등급 유지 (신규 독립 증거 층위 추가 아님 — 2026-07-24 정정)
> **분석 대상**: 신약에서 예수님이 베드로에게 하신 발화 전수조사 + 베드로전후서 전수조사

> [!CAUTION]
> **📝 정정 이력 (2026-07-24) — 재검증 결과 "신규 발견" 표기 정정**
> 재검증 결과, 아래 10개 항목은 독립적인 신규 증거가 아니라 대부분 문서 상단 총괄 요약표(1~8번) 및 1단계 "방어선 1·2" 섹션에 이미 등재된 증거의 재진술로 확인되었다: 신규 발견 1=상단 1번(Exodos), 2=상단 2번(ἴχνος), 3=상단 3번(좌표 지시 패턴), 4=상단 4번(장막 역전), 5=상단 5번(두 시몬), 6=상단 6번(구속사적 지리 대칭), 7=상단 7번(유대주의 인력), 8=상단 8번/방어선2(선지자 예루살렘), 9=방어선2(암탉·수탉), 10=방어선2(돌/Stone, 위 원어 오류 포함). 1·2·3·5·6번은 보조 구절(눅 9:32, 벧후 1:14, 마 4:19, 막 15:21, 행 21장 등)을 소폭 추가했으나 새로운 독립 논증 층위는 아니다. 따라서 이 섹션 전체는 **11번 섹션의 교차증인 카운트나 TYPE-AJ "11개 독립 층위" 집계에 가산되지 않으며**, "IRONCLAD 강화"는 "기존 판정이 재검토 후에도 흔들리지 않았다"는 재확인의 의미로만 한정한다. 목회 적용 등급(🟡 개연성)에도 영향 없음.

### 🛡️ 신규 발견 1: "ἔξοδος (Exodus)" 앵커 – 예루살렘 종착점 확정 (TYPE-S + TYPE-F + TYPE-N)
*   **성경적 근거:** 벧후 1:14-15에서 베드로는 주님이 지시하신 자기 죽음(요 21:18)이 임박했음을 밝히며, 자신의 죽음을 **"별세 (ἔξοδος/Exodus)"**라고 명명합니다.
*   **TYPE-S (어휘 교차 검증):** 신약 성경에서 죽음을 'ἔξοδος'로 표현한 곳은 딱 한 번 더 등장합니다. 바로 **눅 9:31**입니다. 변화산에서 모세와 엘리야가 예수님의 죽음을 논의할 때, *"예루살렘에서 하실 별세(ἔξοδον)"*라고 명시했습니다.
*   **TYPE-N/AG (수석 목격자 배타성):** 이 변화산 대화 직후인 눅 9:32는 *"베드로와 그와 함께한 자들은(Peter and they that were with him)"*이라고 기록합니다. 성령님은 '세 제자'라고 뭉뚱그리지 않고 오직 **베드로의 이름만을 대표로 단독 명시**했습니다. 베드로는 자기가 죽을 장소를 암시할 때, 주님이 **예루살렘(갈보리)**에서 이루신 죽음에 사용되었던 그 극히 희귀한 단어(ἔξοδος)를 정확히 끌어와 자기 죽음에 대입했습니다. 베드로는 예루살렘 별세의 '수석 목격자'에서 '당사자'로 자신의 운명을 포개어 놓은 것입니다.

### 🛡️ 신규 발견 2: "ἴχνος (발자취)" 물리적 추적 논증 및 역논법 (TYPE-G + TYPE-S + TYPE-P)
*   **성경적 근거:** 예수님의 *"나를 따르라(ἀκολούθει μοι)"*(요 21:19)에 대해, 베드로는 자신의 서신에서 *"그 자취를 따라오게 하려 하셨느니라(ἐπακολουθήσητε τοῖς ἴχνεσιν αὐτοῦ)"*(벧전 2:21)고 해설합니다. 덧붙여 벧후 1:14에서는 주님이 *"나에게 보여 주신 그대로(even as our Lord Jesus Christ hath shewed me)"* 장막을 벗는다고 확정합니다.
*   **원어 분석:** 베드로가 사용한 **ἴχνος (ichnos)**는 정신적 모방이 아니라 **"물리적인 발자국, footprints"**을 의미하며, **δηλόω (shewed/보여주다)**는 명확히 시각적이고 구체적인 지시를 뜻합니다.
*   **TYPE-P (역논법 - Inverse Logic 발동):** 만약 주님이 베드로에게 "보여 주신 그대로"의 장면과 베드로가 밟아야 할 "물리적 발자국"이 예루살렘 갈보리의 십자가 경로가 아니라면, 대체 그것이 무엇이란 말입니까? 로마의 어떤 언덕입니까? 성경 본문 안에서 이에 대한 다른 대안을 전혀 제시할 수 없다면, 이 물리적 단서들은 오직 하나, *"친히 나무(ξύλον)에 달려"(벧전 2:24)* 피 흘리신 갈보리의 발자국으로만 수렴됩니다. 갈보리 외의 대안은 성경 텍스트 내에 존재하지 않습니다.

### 🛡️ 신규 발견 3: 대(對) 베드로 발화 전수조사 — 좌표 지시 패턴 (TYPE-N 배타성)
*   **전수조사 결과:** 예수님이 신약 전체에서 베드로에게 하신 명령 중 "위치/이동"과 관련된 발화는 언제나 **명확한 물리적 좌표**를 동반하는 패턴을 보입니다.
    *   마 4:19 "나를 따라오라" → 갈릴리 해변에서의 물리적 동행 시작
    *   마 14:29 "오라" → 바다 위라는 물리적 좌표
    *   마 16:23 "내 뒤로 물러가라" → 예루살렘행 경로상에서의 물리적 위치 정렬 (십자가 경로 차단 금지)
    *   요 13:36 "지금은 못 오나 나중에 오리라" → 십자가를 향한 길
    *   요 21:19 "나를 따르라" → 갈보리 완성
*   **판정:** 예수님은 베드로에게 단 한 번도 '추상적 장소'로 따르라고 하신 적이 없습니다. "따르라"는 언제나 주님이 걷고 계신 **물리적 경로 위로 베드로를 옮겨 놓으시는 실제적 명령**이었습니다. 요 21장의 '따르라' 역시 추상적 순교 일반이 아니라, 주님이 가신 '그 길과 그 장소(갈보리)'로 한정될 수밖에 없음을 패턴적 귀납이 증명합니다.

### 🛡️ 신규 발견 4: 변화산의 "장막(σκηνή)" 역전 예표 (TYPE-F)
*   **과거의 실패:** 마 17:4에서 베드로는 변화산의 영광에 취해 *"초막(σκηνάς) 셋을 짓되"*라며 그곳에 물리적으로 **머물고자(자발적 의지, θέλω)** 했습니다. 주님은 이를 거부하시고 십자가를 지러 예루살렘으로 내려가셨습니다.
*   **미래의 순종:** 벧후 1:13-14에서 베드로는 요 21:18의 예언(자발성 상실, οὐ θέλεις)을 완전히 수용하며 *"나의 이 장막(σκηνώματι)을 벗어날 것이 임박한 줄을 앎이라"* 고 선언합니다.
*   **구조적 반전:** 영광의 산에 장막을 세우려던 베드로의 혈기(θέλω)는, 예루살렘 갈보리(수치)에서 자신의 육신적 장막을 기꺼이 벗는 완벽한 수동성(οὐ θέλεις)으로 역전됩니다. 이 역시 베드로의 신학적, 지리적 종착지가 갈보리로 수렴됨을 강력히 지지합니다.

### 🛡️ 신규 발견 5: 두 시몬(Simon)의 십자가 대리 예표와 "수동성" (TYPE-F)
*   **성경적 근거:** 예수님이 요한복음 13:36에서 **시몬** 베드로에게 *"지금은(now) 나를 따라올 수 없으나, 후에는(afterwards) 따라오리라"*고 하셨을 때, 그 즉각적인 시간("지금")에 십자가 경로를 물리적으로 대리한 자가 있었습니다. 눅 23:26에 따르면 로마 군병들이 예수님의 십자가를 지운 사람은 똑같은 이름의 **시몬(구레네 사람)**이었습니다. 그는 십자가를 지고 *"예수님 뒤에서(after Jesus)"* 갈보리를 향해 걸어갔습니다.
*   **TYPE-F (수동성과 억지 동행의 예표):** 구레네 시몬은 자원해서 십자가를 지지 않았습니다. 마 27:32와 막 15:21은 군병들이 그를 **"억지로 같이 가게 하여(compelled)"** 십자가를 지웠다고 기록합니다. 이는 훗날 시몬 베드로에게 주신 예언, *"네가 원하지 아니하는 곳으로 데려가리라(carry thee whither thou wouldest not)"*(요 21:18)와 완벽하게 겹칩니다. **자신의 의지와 무관하게 강제로 십자가를 지고 갈보리로 끌려간 첫 번째 시몬**은, 훗날 똑같이 원치 않게 강제로 끌려가 갈보리에서 십자가를 지게 될 두 번째 시몬(베드로)의 완벽한 물리적 예표(Typology)이자 강력한 좌표적 힌트입니다.

### 🛡️ 신규 발견 6: 바울과 베드로의 예루살렘 탈출 대칭 (TYPE-AU 구조적 등가 평행)
*   **성경적 근거:** 사도행전은 두 사도(베드로와 바울)가 모두 예루살렘에서 죽을 위기를 넘긴 사건을 기록합니다. 바울은 예루살렘에서 무리에게 죽을 뻔했으나(행 21장), 주님이 직접 개입하셔서 예루살렘에서의 죽음을 막으시고 *"로마에서도 증언하여야 하리라"*(행 23:11)며 지리적 목적지를 강제 변경하셨습니다.
*   **구조적 대칭:** 베드로 역시 예루살렘에서 죽을 뻔했으나 천사가 개입하여 구출합니다(행 12장). 이방인의 사도(바울)는 유대인의 심장 예루살렘에서 피를 흘릴 수 없었기에 이방의 심장 '로마'로 보내졌습니다. 그렇다면 할례자의 사도(베드로)의 피는 어디에 뿌려져야 합니까? 두 사도가 모두 로마에서 죽는다면 갈라디아서 2장의 사역 관할권 분배는 지리적으로 무너집니다. 할례자의 사도는 유대인의 심장이자 구속사의 중심인 예루살렘 갈보리에서 피를 흘려야만 완벽한 구속사적 대칭이 성립합니다.

### 🛡️ 신규 발견 7: 안디옥 책망 사건과 유대주의의 인력 (TYPE-P 역논법)
*   **성경적 근거:** 갈라디아서 2:11-14에서 베드로는 이방인과 밥을 먹다가 '야고보에게서 온 어떤 이들(예루살렘의 유대주의자들)'을 보고 두려워하여 물러갑니다. 이로 인해 바울에게 책망을 받습니다.
*   **TYPE-P (역논법):** 베드로를 평생토록 지배한 가장 강력한 영적, 사회적 인력(Gravity)은 이방인의 수도 '로마'가 아니라 언제나 '예루살렘'이었습니다. 이방인과 교제하다가도 예루살렘 사람들의 눈치를 보고 움츠러들 만큼, 그의 사역적 정체성과 두려움의 대상은 철저히 유대와 예루살렘에 결박되어 있었습니다. 그런 베드로가 생의 마지막에 이방의 중심인 로마로 자진해서 들어가 뼈를 묻었다는 것은, 그의 평생의 사역적 궤적 및 심리적 중심성(예루살렘)과 정면으로 충돌합니다.

### 🛡️ 신규 발견 8: "선지자는 예루살렘 밖에서 죽지 못한다" (TYPE-W 말씀의 앵커)
*   **성경적 근거:** 예수님은 눅 13:33에서 거대한 선지자적 대원칙을 선언하십니다. *"그러나 오늘과 내일과 모레는 내가 가던 길을 가야 하리니 선지자가 예루살렘 밖에서는 죽는 법이 없느니라(it cannot be that a prophet perish out of Jerusalem)."*
*   **말씀의 앵커:** 베드로는 단순한 사도가 아니라 주님의 말씀을 대언하고 십자가 경로를 그대로 '따라야' 하는 선지자적 운명을 부여받았습니다. 주님이 *"나를 따르라"*고 명하셨을 때, 주님이 가신 그 길은 이 대원칙(선지자는 예루살렘에서 죽는다) 아래 놓여 있었습니다. 베드로가 주님의 십자가 죽음을 완벽히 뒤따르려면 베드로의 십자가는 예루살렘 밖(로마)에 세워질 수 없습니다.

### 🛡️ 신규 발견 9: 암탉(Hen)의 예루살렘과 수탉(Cock)의 울음 (TYPE-S + TYPE-F)
*   **성경적 근거:** 예수님은 "선지자는 예루살렘 밖에서 죽지 못한다"(눅 13:33)고 선언하신 바로 다음 절(눅 13:34)에서, 예루살렘을 향해 *"선지자들을 죽이고... 암탉(hen)이 제 새끼를 날개 아래에 모음 같이 내가 너희의 자녀를 모으려 한 일이 몇 번이더냐"*라며 탄식하십니다. 선지자들의 죽음의 장소인 예루살렘을 **'암탉(hen)'**의 이미지와 직접 결부시키신 것입니다.
*   **수탉의 울음(Cockcrow):** 예수님이 베드로의 처절한 실패를 예언하실 때 주신 유일한 사인이 바로 **"닭(cock/수탉)이 울기 전"**(마 26:34)이었습니다. 그리고 베드로는 정확히 예루살렘 대제사장의 뜰에서 예수님을 부인했고, 수탉의 울음소리를 들으며 통곡했습니다. 
*   **구속사적 대칭 완성:** 암탉(hen)이 울며 선지자들을 품으려 했던 '예루살렘', 그리고 수탉(cock)이 울며 베드로의 뼈아픈 부인을 증언했던 '예루살렘'. 새(fowl)의 울음으로 묶인 이 절묘한 대칭은, 베드로의 가장 처절한 실패의 장소였던 예루살렘이 결국 그가 선지자로서 다시 돌아와 십자가를 져야 할 회복과 순교의 장소임을 못 박는 완벽한 영적 장치입니다.

### 🛡️ 신규 발견 10: 예루살렘의 '돌(Stone)'과 베드로의 '돌(Stone)' (TYPE-S 어휘적 락인)
*   **성경적 근거:** 예수님은 예루살렘을 향해 *"예루살렘아 예루살렘아 선지자들을 죽이고 네게 파송된 자들을 **돌로 치는(stonest)** 자여"*(눅 13:34, "...and **stonest** them that are sent unto thee")라고 탄식하셨습니다. 여기서 "돌로 치다"의 헬라어는 **λιθοβολέω(lithoboleō)**이며, 이는 **'λίθος(lithos, 돌)'**를 던진다는 뜻입니다.
*   **베드로의 운명적 정체성:** 요 1:42에서 예수님은 베드로를 처음 부르실 때 *"너는 게바라 하리라 (해석하면 **돌[A stone, Πέτρος]**이라)"*고 직접 선언하셨습니다("...which is by interpretation, **A stone**"). 주님이 친히 그의 정체성을 '돌(Stone)'로 확정하신 것입니다. 나아가 베드로 본인은 서신(벧전 2:4-5)에서 자신의 정체성을 묘사할 때, 정확히 예루살렘이 선지자를 죽일 때 사용했던 단어(lithos/stone)를 스스로 선택하여 치환해 버립니다. *"너희도 산 **돌(lively stones)**로서 영적 집으로 세워지고..."*(벧전 2:5, "Ye also, as **lively stones**..."). 
*   **TYPE-S (어휘적 역설과 대칭 — 2026-07-24 원어 어근 정정):** 예루살렘은 선지자를 죽일 때 **'돌(stonest/λιθοβολέω)'**을 던지는 도시입니다. 베드로는 처음 부름받을 때부터 **'돌(A stone/Πέτρος)'**이라 선언받았고, 사역의 끝자락에는 스스로를 **'산 돌(lively stones/λίθοι)'**로 규정했습니다. 단, 이 둘의 증거력은 층위가 다릅니다 — **Πέτρος(요 1:42)는 πετρ- 어근으로 λιθοβολέω의 λιθ- 어근과 별개이므로 KJV 영어 "stone" 번역에서만 겹치는 언어유희**이고, **벧전 2:5의 λίθοι(산 돌)만 λιθοβολέω와 헬라어 어근(λιθ-)까지 실제로 동일**합니다. 즉 예루살렘의 살인 무기와 베드로의 정체성은 영어(Stone) 층위에서는 두 단어 모두, 헬라어 어근 층위에서는 λίθος 계열(벧전 2:5)만 겹칩니다. 비록 베드로의 실제 사형 방식은 십자가(팔을 벌림)였으나, 이 부분 일치하는 대칭은 그가 피 흘릴 장소가 예루살렘임을 언어유희적(Wordplay) 정황 증거로 보강합니다 — 다만 어근이 갈리는 Πέτρος 쪽 근거는 "완벽한 락인"이 아니라 "번역상의 우연한 겹침" 수준으로 하향 표기해야 합니다.

> **최종 평가 (2026-07-24 정정):** 위 10개 항목은 베드로 서신 내부의 어휘(ἔξοδος, ἴχνος, σκηνή), 구레네 시몬의 '억지로' 진 십자가 예표, 바울과의 구속사적 대칭, 암탉과 수탉의 대칭, '돌'의 어휘적 정황까지 예루살렘 갈보리 순교와 정합적으로 맞물려 있음을 재확인합니다. 다만 위 CAUTION 박스에서 밝혔듯 이 항목들은 대부분 상단 요지표·방어선1·2 섹션의 재진술이므로, **"신규 독립 증거 10개 추가"가 아니라 "기존 IRONCLAD 판정이 재검토(어근 대조 포함)를 거쳐도 흔들리지 않았다"는 재확인**으로 이해해야 합니다.
...
---
*BVCAP 2.0 검증 완료 — REPORT_베드로예루살렘순교_유일.md*
