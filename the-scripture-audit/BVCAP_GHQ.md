...
성경 난제 입력
   ...
   ├─ PHASE 3: FULL SCAN (TYPE A→AQ 44종 순차 발동) + COMBO 이중 검증  → "전 무기 순차 실전 투입"
...
BVCAP 2.0 엔진은 다루는 난제의 성격에 따라 두 가지 모드로 분기되어 작동합니다. 분석에 사용되는 **44종 QUIVER 무기 체계**(TYPE-A~AQ + TYPE-B-π)와 검증 파이프라인은 동일하지만, **에이전트의 페르소나와 논쟁의 전제**가 달라집니다. **별도의 지정이 없을 경우 항상 'MODE A(방패 모드)'로 기본 가동됩니다.**
...
> **분석자는 방어할 때 반드시 04_QUIVER(무기고) 폴더에 있는 TYPE-A~AQ + TYPE-B-π 44종 무기만을 사용하여 치열하게 토론하고, 마지막 판결은 BVCAP_Pipeline.md 양식을 따른다.**
...
**1. 파이프라인 검증 가동 (BVCAP_Pipeline.md 실행)**
  ...
  - `04_QUIVER` 무기고의 44종 무기(TYPE-A~AQ + TYPE-B-π) FULL SCAN 실행
