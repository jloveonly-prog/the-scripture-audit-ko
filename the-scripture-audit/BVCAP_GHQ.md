...
성경 난제 입력
   ...
   ├─ PHASE 3: FULL SCAN (TYPE A→AQ 전종 순차 발동) + COMBO 이중 검증  → "전 무기 순차 실전 투입"
...
BVCAP 2.0 엔진은 다루는 난제의 성격에 따라 두 가지 모드로 분기되어 작동합니다. 분석에 사용되는 **QUIVER 무기 체계 전종**(TYPE-A~AQ + TYPE-B-π)과 검증 파이프라인은 동일하지만, **에이전트의 페르소나와 논쟁의 전제**가 달라집니다. **별도의 지정이 없을 경우 항상 'MODE A(방패 모드)'로 기본 가동됩니다.**
...
> | **Phase 2: 모순 명세서** | **GATE 2:** 주석 검색 금지 및 명제화 | 학계 통설 격리, 원문 기반 세팅 |
> | **Phase 3: FULL SCAN** | **GATE 3:** SKILL(TYPE 무기) 실행 | QUIVER 전종 순차 발동 |
> | **Phase 4: 회피논법 탐지** | **GATE 4:** 역산 교차 검증 | 오류 및 비약 차단 |
> | **Phase 5: 중립 판정** | **GATE 5:** 마스터피스 보고서 출력 | 최종 결론 및 비유(Analogy) 도출 |
...
> [!NOTE]
> **분석자는 방어할 때 반드시 04_QUIVER(무기고) 폴더에 있는 TYPE-A~AQ + TYPE-B-π 전종 무기만을 사용하여 치열하게 토론하고, 마지막 판결은 BVCAP_Pipeline.md 양식을 따른다.**
...
| 공인 COMBO 2종 | 2개 TYPE 동시 동일 결론 | ✅✅ CONFIRMED |
| 공인 COMBO 3종+ | 3개 이상 TYPE 수렴 + STRESS-TEST-7 통과 | ✅✅✅ IRONCLAD |

> **공인 COMBO 전종 목록 및 상세 발동 조건**: `BVCAP_Pipeline.md` → **COMBO-VERIFY** 섹션 참조 (26종 등록)
...
**1. 파이프라인 검증 가동 (BVCAP_Pipeline.md 실행)**
  ...
  - `04_QUIVER` 무기고의 전종 무기(TYPE-A~AQ + TYPE-B-π) FULL SCAN 실행
