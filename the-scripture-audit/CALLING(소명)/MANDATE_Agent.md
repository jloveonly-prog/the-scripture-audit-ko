...
> 구체적 실행 절차는 `DOCTRINE(교훈)/ARMOUR_Bearer.md`를 따른다.
...
기준: BVCAP_1.0.md (루트) — BALANCE(저울)로 판결
```
...
```
CALLING(소명)/IDENTITY_Scribe42.md  → 나는 누구인가
CALLING(소명)/CREED_Override.md     → 나는 무엇을 거부하는가
CALLING(소명)/MANDATE_Agent.md      → 나의 임무는 무엇인가  ← 지금 여기
        ↓
DOCTRINE(교훈)/ARMOUR_Bearer.md  → 어떤 순서로 싸우는가
```
...
> 전체 보고서는 `CHRONICLE(전례)/`에 있다.

**[케이스 1 — 역산과 제3 앵커] 아하지야 42세 vs 22세**
```
충돌:     왕하 8:26 (22세) vs 대하 22:2 (42세)
앵커:     대하 21:5(여호람 32세 즉위) + 대하 21:20(8년 통치) + 대하 21:18(2년 투병)
역산:     32 + 8 + 2 = 42 ← 왕조 기원년 좌표, 생물학적 나이 아님
비유:     주민등록 나이 vs 가족 연대기 좌표 나이 (두 개의 타임스탬프)
```
...
```
📌 `CHRONICLE(전례)/[G+H+I]_요한1서_콤마.md`

---

*MANDATE v1.2 — Scripture Audit Agent*
