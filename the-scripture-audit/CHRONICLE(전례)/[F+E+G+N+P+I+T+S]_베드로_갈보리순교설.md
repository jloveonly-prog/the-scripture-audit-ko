...
| **"afterwards" (KJV)** | 단순히 나중에 = 막연한 미래 | ὕστερον = "그 이후에" — 예수님 부활 승천 이후 특정 시점을 가리키는 시간 부사 | ❌ 오독 |

> **TYPE-T 판정**: ἀκολουθέω는 복음서 전체에서 **물리적 동행**의 동사이며, 시제 ἀκολουθήσεις는 명령이 아닌 **미래 예언**이다.
> "영적 모방"으로 해석하면 이 미래 직설법이 단순 권고로 전락하여 예언적 선언의 성격을 소멸시킨다.
> 어휘(ἀκολουθέω = 물리적 동행) + 시제(미래 직설법 = 예언적 확정) 두 층위에서 동시에 오독이 발생함을 적발.
...
> **핵심 원리:** 예수님은 요 13:33에서 다른 모든 제자들에게 "너희는 내가 가는 곳에 올 수 없다"고 선언하셨다. 오직 베드로에게만 "따라올 것"을 허락하셨다. **어떤 해석이든 이 '배타성'을 파괴한다면 그것은 오독이다.**

| 해석의 가정 | 배타성 유지 여부 검증 | 결과 |
|:---|:---|:---:|
| "가는 곳" = **천국** | 다른 제자들도 믿음으로 천국에 가므로, 베드로만의 배타성이 파괴됨 | ❌ 모순 |
| "가는 곳" = **일반적 순교** | 야고보(행 12:2)를 비롯해 다른 제자들도 순교했으므로, 이 역시 배타성 파괴 | ❌ 모순 |
| "가는 곳" = **십자가형** | 역사 전승에 의존해야 하며, 안드레 등도 십자가형을 받았다는 전승과 충돌 위험 | ⚠️ 불안정 |
| "가는 곳" = **갈보리 (특정 장소)** | 예수님이 죽으신 바로 그 장소에서 십자가형을 당한 제자는 **오직 베드로뿐임** | ✅ **성립** |

> **판정:** "내가 가는 곳"을 '일반적 순교'나 '천국'으로 넓게 해석하면 요 13:33의 예수님 말씀이 거짓말이 되는 치명적 자가모순이 발생한다. **해석이 '갈보리'라는 특정한 지리적 장소로 축소(Narrowing)될 때에만, 요 13:33-36의 모든 말씀이 모순 없이 절대적 진리로 성립한다.** 따라서 이것은 단순한 추론이 아니라, **배타성 유지를 위한 유일한 논리적 귀결**이다.
...
> **이 섹션은 BVCAP_Skill_Pipeline.md v2.8 COMBO-VERIFY 단계의 실체 검증 기록입니다.**
