...
의무 수집 항목:
  ...
  ⑤ [화자·장르 태그 부착 — 필수, 2026-07-23 신설] 수집된 모든 구절(충돌 구절 + 앵커 + 병행)에
     ...
     → 근거: `System_Architecture(시스템_설계원리)/20260722_중재자 편향성_사례.md` — 화자 구분 없이 전수조사하여
        판정이 왜곡되었던 사건(DEF-01 발화자 구분 방벽 100% 봉쇄 실패).
...
[STEP 2.5] 반례 자동탐색 (Counter-Example Auto-Search) — 2026-07-23 신설, 의무 실행
  ...
  → 근거: `System_Architecture(시스템_설계원리)/20260722_중재자 편향성_사례.md` — 창17:5 반례를 AI가
     스스로 찾지 못하고 사용자가 제시한 뒤에야 인정한 사건 (VER-01 반례 탐색 의무 미이행).
...
[GATE: 판정 다운그레이드/번복 전 필수 검문소 — OVERRIDE-2 연동, 2026-07-23 신설 (대칭 방어선)]
  ...
  → 두 검문소를 통과한 경우에만 판정 변경이 확정되며, 변경 사유(어떤 TYPE/앵커가 뒤집혔는지)를
     보고서에 명시한다.
  → 근거: `System_Architecture(시스템_설계원리)/20260722_중재자 편향성_사례.md` — 신규 증거 없이
     대화 압박만으로 IRONCLAD→TENTATIVE, 95점→45점 등 판정이 4회 이상 요동친 사건 (ERR-A03/A02/A04).
