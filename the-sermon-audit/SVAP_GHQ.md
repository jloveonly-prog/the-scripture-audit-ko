...
설교 원고 입력 (_INBOX)
   ...
   └─ GATE 9: 실전 투입 변환 (Deployment Conversion)         → "이걸 실제 댓글창에서 어떤 순서로 꺼내는가?"
         ...
         └─ 결과 → **PART D(실전)** 작성, PART A+B+C+D를 합쳐 **단일 파일**로 02_REPORT 폴더에 `AUDIT_` 접두사로 저장
...
| 🎯 **투입자** (Deployer) | **GATE 9 담당** | 실전 지휘관 | PART C의 🟢·🟡 논거를 **댓글창에서 그대로 붙여넣을 수 있는 대화 트리**로 변환. 망치 1문장 선정 → 상대 예상 답변별 후속 수 준비 → 자기모순 함정 우선 배치 → 투입 금지 논거 격리. **PART D로 첨부.** |
...
│  - 투입 금지 목록 격리                        │
│  - PART D 작성 → A+B+C+D 단일 파일 02_REPORT  │
└─────────────────┬───────────────────────────┘
         │
         ▼
   ┌─────┴─────┐─────────┐
...
> 실제 작성 사례가 필요하면 **완성된 보고서를 참조한다** — 예: `02_REPORT(설교감사보고서)/AUDIT_브라이언덴링거_신격교리_20260812.md` §16(실전 반론 스크립트 아카이브). 이 PART D 양식 자체가 그 §16의 사용 방식을 일반화한 것이다.
...
**6. 실전 투입 변환 (SVAP_GHQ.md 양식 — GATE 9, PART D)**
  ...
  - 자기모순 함정을 최우선 배치한다 (신학적 입장과 무관하게 성립하므로 상대가 교리 방어로 빠져나갈 수 없다)
  - **종합 반론 스트레스 테스트**를 실행한다 — ①카드 간 규칙 일관성 ②독립성(연쇄 붕괴) ③역공 경로. ①에 위반이 남으면 세트를 투입하지 않는다
  - PART A + B + C + D를 **하나의 파일**로 합쳐 `AUDIT_[설교명]_[날짜].md` 이름으로 `02_REPORT(설교감사보고서)` 폴더에 저장 (별도 파일로 쪼개지 않는다)
